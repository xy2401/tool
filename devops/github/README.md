...
* **实时动态渲染**：基于底层代码生成的 `${key}` 替换语法。左侧无论如何翻页、过滤，右侧都会毫秒级实时生成渲染好的最终排版输出。
* **智能表头提取 (Magical Header Extraction)**：当你在编写 Markdown Table 或是 CSV 模板时，无需专门区分“表头”和“循环体”。模板引擎会自动从上往下扫描，**所有出现在第一个 `${...}` 变量之前的纯静态行，都会被自动识别为“静态表头”**，并且在最终结果中只输出一次。
  * *💡 高阶黑魔法*：如果你的前几行是纯静态文字，但你**希望**它在每个仓库渲染时都被重复输出一遍（不想被当成表头过滤掉），只需在该行任意位置插入一个隐形的空变量 `${''}` 即可。引擎一旦扫描到 `${...}`，就会立刻判定循环体开始，完美绕过检测！
